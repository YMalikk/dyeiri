<!-- Header ================================================== -->
<header>
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12 col-xs-12">
                <div class="pull-right">
                        <a href="#" class="be_delivery_man">Devenir un livreur</a>

                </div>
            </div>
        </div>
        <div class="row">
            <div class="col--md-4 col-sm-4 col-xs-4">
                <a href="index-2.html" id="logo">
                    <img src="{{asset('img')}}/logoDyeiri.png" width="190" height="23" alt=""  data-retina="true" class="hidden-xs logo_image">
                    <img src="{{asset('img')}}/logoDyeiri.png" width="59" height="23" alt="" data-retina="true" class="hidden-lg hidden-md hidden-sm logo_image_mobile">
                </a>
            </div>
            <nav class="col--md-8 col-sm-8 col-xs-8 header_dyeiri">
                <a class="cmn-toggle-switch cmn-toggle-switch__htx open_close" href="javascript:void(0);"><span>Menu mobile</span></a>
                <div class="main-menu">
                    <div id="header_menu">
                        <img src="{{asset('img')}}/logoDyeiri.png" width="190" height="23" alt="" class="logo_image_menu" data-retina="true">
                    </div>
                    <a href="#" class="open_close" id="close_in"><i class="icon_close"></i></a>
                    <ul>

                        <li><a href="about.html">Blog</a></li>
                        <li><a href="about.html">A propos de nous</a></li>

                        <li><a href="#0" data-toggle="modal" data-target="#login_2">Connexion</a></li>
                        <li><a href="http://themeforest.net/item/quickfood-delivery-or-takeaway-food-template/13958100?ref=ansonika">Devenir un chef</a></li>

                    </ul>
                </div><!-- End main-menu -->
            </nav>
        </div><!-- End row -->
    </div><!-- End container -->
</header>
<!-- End Header =============================================== -->
   